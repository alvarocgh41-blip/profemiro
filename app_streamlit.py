"""
ProfeMiro v6.1 — Cambios respecto a v6:
  - Biblioteca: valor por defecto "Ninguna" (no muestra quizzes hasta elegir sección)
  - Launcher: mismo comportamiento, "Ninguna" por defecto
  - Crear sección: botón independiente en Biblioteca (no hay que crear un quiz)
  - Registro de nuevo profesor: sala con 3 dígitos aleatorios sin colisiones (ej: SALA_042)
  - CSV descarga: nota en % en vez de número de preguntas acertadas
"""
...
if "_accion_bd" in st.session_state:
    ...
    try:
        ...
        # ── NUEVO v6.1: crear sección independientemente de un quiz ──
        elif _accion["tipo"] == "crear_seccion":
            _nombre_sec = _accion["nombre"].strip()
            if not _nombre_sec:
                st.session_state["_error_msg"] = "El nombre de la sección no puede estar vacío."
            else:
                # Verificar que no exista ya
                _cur.execute(
                    "SELECT COUNT(*) AS n FROM quizzes WHERE id_profesor=%s AND seccion=%s",
                    (_profe_id, _nombre_sec)
                )
                _ya = (_cur.fetchone() or (0,))[0]
                if _ya:
                    st.session_state["_error_msg"] = f"Ya existe una sección llamada '{_nombre_sec}'."
                else:
                    # Crear un quiz fantasma invisible para anclar la sección,
                    # o simplemente guardarlo en session para que aparezca en el widget.
                    # La forma más limpia: insertar un quiz con título vacío marcado como plantilla.
                    # Pero mejor: crear la sección via un quiz placeholder que el profe puede eliminar.
                    # En realidad las secciones existen solo como campo en quizzes,
                    # así que sin un quiz no hay sección. Guardamos en sesión para que aparezca
                    # en el selectbox de sección cuando cree un quiz.
                    # Solución: insertar la sección en una tabla propia si existe, o en quizzes con placeholder.
                    # Para no cambiar el schema, guardamos en session_state como sección disponible.
                    _secs_custom = st.session_state.get("_secciones_custom", [])
                    if _nombre_sec not in _secs_custom:
                        _secs_custom.append(_nombre_sec)
                    st.session_state["_secciones_custom"] = _secs_custom
                    st.session_state["_ok_msg"] = f"Sección '{_nombre_sec}' creada. Asígnala a un quiz al crearlo."
...
def render_topbar_profe(nombre_sala=None, tab="launcher"):
    ...
    with col_logo:
        st.markdown(
            ...
            f'border:2px solid #a78bfa;letter-spacing:0.5px;">v6.1</span>'
            f'&nbsp;{chip}</div>',
            unsafe_allow_html=True)
    with col_nav:
        c1, c2 = st.columns(2)
...
def _get_secciones_disponibles(profe_id):
    """Devuelve las secciones existentes en BD + las creadas manualmente en sesión."""
    try:
        db = conectar(); cur = db.cursor(dictionary=True)
        cur.execute(
            "SELECT DISTINCT seccion FROM quizzes WHERE id_profesor=%s AND seccion IS NOT NULL AND seccion!='' ORDER BY seccion",
            (profe_id,)
        )
        secs_bd = [r["seccion"] for r in cur.fetchall()]
        db.close()
    except:
        secs_bd = []
    secs_custom = gs("_secciones_custom", [])
    todas = list(dict.fromkeys(secs_bd + secs_custom))  # deduplicar preservando orden
    return sorted(todas)

def widget_seccion(profe_id, key_prefix=""):
    secs = _get_secciones_disponibles(profe_id)
...
def pg_alumno_join():
    ...
    st.markdown(f'<div class="join-logo">{logo_svg(72)}<div style="color:white;font-size:2.4rem;font-weight:900;margin:10px 0 4px;letter-spacing:-1px;">ProfeMiro'
                ...
                f'border:2px solid rgba(255,255,255,0.5);letter-spacing:0.5px;vertical-align:middle;">v6.1</span>'
    ...
    if st.button("Crear mi cuenta",use_container_width=True,type="primary",key="reg_btn"):
        if not (nombre.strip() and email.strip() and password): ss("_reg_error","Rellena todos los campos.")
            ...
            try:
                ...
                if cur.fetchone():
                    db.close(); ss("_reg_error","Este email ya esta registrado.")
                else:
                    # ── FIX v6.1: código de sala de 3 dígitos sin colisiones ──
                    usados = set()
                    cur.execute("SELECT nombre_sala FROM profesores")
                    for row in cur.fetchall():
                        usados.add(row["nombre_sala"])
                    cur.execute("SELECT nombre_sala FROM salas_profesor")
                    for row in cur.fetchall():
                        usados.add(row["nombre_sala"])
                    ...
                    sala_code = None
                    while intentos < 100:
                        candidato = f"SALA_{random.randint(0, 999):03d}"
                        if candidato not in usados:
                            sala_code = candidato
                            break
                        intentos += 1
                    if not sala_code:
                        # Fallback: 4 dígitos si los 3 están agotados
                        while True:
                            candidato = f"SALA_{random.randint(0, 9999):04d}"
                            if candidato not in usados:
                                sala_code = candidato
                                break
                    ...
                    db.commit(); db.close()
                    ss("_reg_ok",f"Cuenta creada! Tu sala es: {sala_code}")
...
def pg_launcher():
    ...
    # ── Lanzar examen — filtro por sección con "Ninguna" por defecto ──
    ...
    # ── FIX v6.1: "Ninguna" como valor por defecto en Launcher ──
    sec_opts = ["Ninguna"] + secciones_disp
    sec_sel_l = gs("launcher_sec_filtro", "Ninguna")
    if sec_sel_l not in sec_opts: sec_sel_l = "Ninguna"
    ...
    if not quizzes:
        st.markdown('<div style="text-align:center;padding:30px;color:#9ca3af;background:white;'
                    ...
                    unsafe_allow_html=True)
    elif sec_sel_l == "Ninguna":
        # No mostrar nada hasta que se elija sección
        st.markdown('<div style="text-align:center;padding:30px;color:#9ca3af;background:white;'
                    'border-radius:16px;border:1px solid #e5e7eb;">'
                    '<div style="font-size:2rem;">☝️</div>'
                    '<div style="font-weight:700;">Elige una sección para ver los cuestionarios disponibles</div>'
                    '</div>', unsafe_allow_html=True)
    else:
        quizzes_f = quizzes if sec_sel_l == "Sin seccion" else [
            q for q in quizzes if (q.get("seccion") or "Sin seccion") == sec_sel_l
        ]
        if not quizzes_f:
            st.info(f"No hay cuestionarios en la sección '{sec_sel_l}'.")
...
def pg_biblioteca():
    ...
    # ── NUEVO v6.1: Crear sección independientemente ──────────────
    with st.expander("📁 Gestionar secciones", expanded=False):
        secs_existentes = _get_secciones_disponibles(profe_id)
        if secs_existentes:
            st.markdown(f'<div style="color:#6b7280;font-size:.85rem;margin-bottom:8px;">Secciones existentes: {", ".join(secs_existentes)}</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div style="color:#9ca3af;font-size:.85rem;margin-bottom:8px;">No hay secciones creadas aún.</div>', unsafe_allow_html=True)
        col_ns, col_nb = st.columns([3, 1])
        with col_ns:
            nueva_sec_nombre = st.text_input("Nombre de la nueva sección",
                                              placeholder="Ej: Matemáticas ESO1, Física 2ºBach...",
                                              key="bib_nueva_sec_nombre",
                                              label_visibility="collapsed")
        with col_nb:
            if st.button("➕ Crear sección", key="bib_crear_sec", use_container_width=True, type="primary"):
                nombre_limpio = nueva_sec_nombre.strip()
                if not nombre_limpio:
                    ss("_error_msg", "Escribe un nombre para la sección.")
                elif nombre_limpio in secs_existentes:
                    ss("_error_msg", f"Ya existe una sección llamada '{nombre_limpio}'.")
                else:
                    # Guardamos en session_state para que aparezca en los selectboxes
                    secs_custom = gs("_secciones_custom", [])
                    if nombre_limpio not in secs_custom:
                        secs_custom.append(nombre_limpio)
                    ss("_secciones_custom", secs_custom)
                    ss("_ok_msg", f"Sección '{nombre_limpio}' creada. Ya puedes asignarla a quizzes.")
                st.rerun()

    st.markdown("<br>",unsafe_allow_html=True)

    # ── Lista de quizzes con "Ninguna" por defecto ────────────────
    if not quizzes:
    ...
    # ── FIX v6.1: "Ninguna" como valor por defecto en Biblioteca ──
    sec_opciones = ["Ninguna"] + list(secciones)
    sec_sel_b = gs("bib_sec_filtro", "Ninguna")
    if sec_sel_b not in sec_opciones: sec_sel_b = "Ninguna"
    ...
    # Si "Ninguna", no mostrar quizzes
    if sec_sel_b == "Ninguna":
        st.markdown('<div style="text-align:center;padding:40px;background:white;border-radius:16px;'
                    'border:2px dashed #e5e7eb;color:#9ca3af;">'
                    '<div style="font-size:2rem;margin-bottom:8px;">☝️</div>'
                    '<div style="font-weight:700;">Elige una sección en el desplegable para ver sus cuestionarios</div>'
                    f'<div style="font-size:.85rem;margin-top:6px;">{total_q} cuestionarios en total · {len(secciones)} secciones</div>'
                    '</div>', unsafe_allow_html=True)
        return

    quizzes_fb = quizzes if sec_sel_b == "Sin seccion" else [
        q for q in quizzes if (q.get("seccion") or "Sin seccion") == sec_sel_b
    ]

    if not quizzes_fb:
        st.info(f"No hay cuestionarios en la sección '{sec_sel_b}'.")
        return
...
def pg_editar_quiz():
    ...
    if tipo_nueva_n in ("test2","test4"):
        ...
        rm_nueva = st.text_input("Respuesta modelo", key="ed_add_rm", placeholder="Respuesta esperada...")
    ...
    if st.button("Guardar nueva pregunta",type="primary",use_container_width=True,key="ed_add_save"):
        if not enunc_nueva.strip(): st.error("Escribe el enunciado.")
            ...
            ss("_accion_bd",{"tipo":"añadir_pregunta","id_quiz":id_quiz,"enunciado":enunc_nueva.strip(),
                             "tipo_preg":tipo_nueva_bd,"respuesta_modelo":rm_nueva.strip(),
                             "opciones":ops_nuevas_add if tipo_nueva_n in ("test2","test4") else []})
            st.rerun()
...
def _csv_alumno(n_a, alumnos, preguntas_ord):
    """FIX v6.1: nota en % en vez de número de aciertos."""
    buf = io.StringIO(); ww = csv.writer(buf)
    n_total_preg = len(preguntas_ord)
    ww.writerow(["ALUMNO", "Nota (%)", "Fecha"] + [f"P{i+1}" for i in range(n_total_preg)])
    ...
    fila = [n_a, f"{pct}%", fecha_str]
    for pq in preguntas_ord:
        r = rs.get(pq["id_pregunta"])
        if r:
            punt = float(r.get("puntuacion") or 0)
            # Para test: correcto/incorrecto legible; para corta: puntuación manual
            if pq.get("tipo") == "test":
                fila.append("✓" if punt >= 1.0 else "✗")
            else:
                fila.append(f"{pct}%" if punt > 0 else "—")
        else:
            fila.append("")
    ww.writerow(fila)
...
def _render_detalle_alumno(n_a, alumnos, preguntas_ord):
    ...
    st.markdown(
        ...
        f'padding:4px 14px;font-size:.85rem;font-weight:800;">{pct}% ({puntos:.0f}/{n_total_preg})</span>'
...
def pg_resultados():
    ...
    sesiones_display = list(sesiones)
    ...
    if tabla_abierta:
        ...
        for n_a in nombres:
            rs = alumnos[n_a]["respuestas"]
            puntos = sum(float(r.get("puntuacion") or 0) for r in rs.values())
            pct = round(puntos / n_preguntas * 100) if n_preguntas > 0 else 0
            fecha_str = alumnos[n_a]["fecha"].strftime("%d/%m/%Y %H:%M") if alumnos[n_a]["fecha"] else "—"
            filas_tabla += (f'<tr>'
                            ...
                            f'<td style="padding:10px 16px;text-align:center;color:#6b7280;">{puntos:.0f}/{n_preguntas} preg</td>'
    ...
    # ── FIX v6.1: CSV con % en vez de aciertos crudos ──
    if sel == "— Todos los alumnos —":
        ...
        for n_a in nombres:
            ...
            pct = round(puntos / n_preguntas * 100) if n_preguntas > 0 else 0
            fecha_str = alumnos[n_a]["fecha"].strftime("%d/%m/%Y %H:%M") if alumnos[n_a]["fecha"] else ""
            fila = [n_a, f"{pct}%", fecha_str, sesion_actual["nombre_sesion"]]
            for pq in preguntas_ord:
                r = rs.get(pq["id_pregunta"])
                if r:
                    punt = float(r.get("puntuacion") or 0)
                    if pq.get("tipo") == "test":
                        ...
                        fila.append(f"{round(punt*100/n_preguntas)}%" if punt > 0 else "—")
                else:
                    fila.append("")
    ...
    if sel != "— Todos los alumnos —":
        ...
        for n_a in nombres:
            rs     = alumnos[n_a]["respuestas"]
            puntos = sum(float(r.get("puntuacion") or 0) for r in rs.values())
            pct_l  = round(puntos / n_preguntas * 100) if n_preguntas > 0 else 0
            fecha_str = alumnos[n_a]["fecha"].strftime("%d/%m/%Y %H:%M") if alumnos[n_a]["fecha"] else "—"
            ...
            with col_info:
                st.markdown(
                    ...
                    f'border-radius:20px;padding:3px 12px;font-size:.8rem;font-weight:800;">{pct_l}% ({puntos:.0f}/{n_preguntas})</span>'
...
# ════════════════════════════════════════════════════════════════════
#  CREAR QUIZ DESDE DOCUMENTO / CSV
# ════════════════════════════════════════════════════════════════════
def pg_crear_doc():
